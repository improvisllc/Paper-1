function [x, t, V, LAM, X, Y, T] = trajectory4ferr(x0, y0, lam0, v0, m0, mk, tk, Jk, d, dcd, r, k)
tic
ta = tk;
y10 = v0; y20 = lam0 * pi / 180; y30 = x0; y40 = y0; y50 = m0;

[t1,y1] = aktiv(y10, y20, y30, y40, y50, mk, tk, Jk, d, dcd, ta, r, k);
n=size(y1);
n=n(1);
vp = y1(n,1); lamp = y1(n,2); xp = y1(n,3); yp = y1(n,4); mp = y1(n,5);

if t1(n) == ta
    [t2,y2] = pasiv (vp, lamp, xp, yp, mp, d, ta, r, k);
...
%% Active Part
function [t,y] = aktiv (v0, lam0, x0, y0, m0, mk, tk, Jk, d, dcd, ta, r, k) 
...
[t,y] = ode113(@(t,y)balact(t,y,m0,mk,tk,Jk,d,dcd,r,k),[tstart tfinal],[v0 lam0 x0 y0 m0],options); 
end
%% Passive Part
function [t,y] = pasiv (v0, lam0, x0, y0, mk, d, ta, r, k) 
...
[t,y] = ode113(@(t,y)balpas(t,y,mk,d,r,k),[tstart tfinal],[v0 lam0 x0 y0],options);
end
%% Ballsitics of Active Part
function dydt = balact(t,y,m0,mk,tk,Jk,d,dcd,r,k)
...
Td = Jk/tk;
dydt(1) = Td/y(5)-(1 + k/100)*cxo*rho*y(1)^2*s/(2*y(5))-g*sin(y(2));
...
function dydt = balpas(t,y,mk,d,r,k)
...
dydt(1) = -(1 + k/100)*cxo*rho*y(1)^2*s/(2*mk)-g*sin(y(2));
dydt(2) = - g * cos(y(2))/ y(1);
dydt(3) = y(1) * cos(y(2));
dydt(4) = y(1) * sin(y(2)); 
end
